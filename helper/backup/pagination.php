...
class pagination
	...
	function query_limit()
	{
		...
		//exit;
		global $wpdb;
		$wp_data = $wpdb->get_results($this->query);	
	...
	{	
		...
		$this->page_name = end(explode("/",$_SERVER['REQUEST_URI']));
	...
	function show_links($query_string="")
	{
		...
		if($this->mins  > 1)
		{
		$link = "<span class='".$this->link_class."'><a  href='".$this->page_name.$delimiter.$this->page_no_variable."=".(($this->mins)-1)."".$query_string."'>&laquo; Previous</a></span>&nbsp;&nbsp;";
		echo $link;
		}
			...
			for($i=$this->mins;$i<=$this->maxs;$i++)
		{
			...
			$link = "<a href='".$this->page_name.$delimiter.$this->page_no_variable."=".$i.$query_string."'>".$i."</a>";
			echo $temp_link_start.$link.$this->link_page_end;
			
		}
		if($this->maxs  < $this->total_pages)
		{
		$link = " <span class='".$this->link_class."'><a class='".$this->link_class_current."' href='".$this->page_name.$delimiter.$this->page_no_variable."=".(($this->maxs)+1).$query_string."'>Next &raquo;</a></span>";
		echo $link;
		}
